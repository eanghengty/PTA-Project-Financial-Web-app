...
**Vue 3 SPA with Vue Router.** Views render through `RouterView` in `App.vue`, with the active view key persisted to `localStorage` as `currentView`. Available views: `dashboard`, `table`, `invoice-list`, `monthly-invoice`, `po-received-summary`, `po-supplier-breakdown`, `reminders`, `cost-to-date`, `pl`, `site-status`, `issue-log`, `import-export`, `admin`.
...
**Admin data** (sites, VO categories, PO supplier categories, PO suppliers, scopes, app settings) lives entirely in `localStorage` under the key `globalData`. It is loaded independently by `AdminView.vue` â the main store does not read it except `VOForm.vue` consumes it for autocomplete dropdowns.
...
- Base PO items have their own **Base PO Summary** card (amber-themed) in the dashboard showing total amount, Has PO / No PO split, and a scope Ã category breakdown table.
...
3. **Base PO Summary** (amber card) â total amount + Has PO / No PO pills + scope Ã category breakdown table
...
9. **BOQ Related VO Amount by Scope** â Four-way split per scope: BOQÃHave PO, BOQÃNo PO, Non-BOQÃHave PO, Non-BOQÃNo PO. VO items only.
...
## PO Supplier Breakdown View (`src/components/POSupplierBreakdown.vue`)

Dedicated view (`po-supplier-breakdown`, teal-themed) for supplier-level PO tracking by job. Jobs are synced from Variation records (`siteId`, `siteName`, `jobNumber`) via **Sync Jobs from Variations** and stored in `localStorage` under `poSupplierBreakdownData`.

Each job supports multiple PO breakdown rows with fields:
- `poNumber`
- `supplier`
- `category`
- `amount`
...
- `category` is a separate PO-supplier taxonomy and does not reuse Variation/VO categories (`globalData.voCategories`).
- `supplier` must be selected from Admin `globalData.poSuppliers` (autocomplete via datalist).
- Off-list values are blocked on save.
...
- **Cost to Complete** reads from Site Status `localStorage.siteStatusData` by matching the `"siteId|jobNumber"` key. It is only included when the header toggle is on and a specific month is selected. The value is the sum of Site Status `costEntries` for that month, using `qtyDays Ã qtyHours Ã qtyPeople Ã rate`.
...
**Tab icons:** all eight tab navigation buttons use inline Material Design SVG icons (`fill="currentColor"`, `viewBox="0 0 24 24"`). The `icon` field in the `tabs` array holds raw SVG `<path d="...">` data rendered as `<svg><path :d="tab.icon"/></svg>`. No emoji characters are used in AdminView.

**PO Supplier Categories:** managed in a dedicated Admin tab and saved in `globalData.poSupplierCategories`. This list is separate from Variation/VO categories and is used only by the PO Supplier Breakdown modal category selector.

**PO Suppliers:** managed in a dedicated Admin tab and saved in `globalData.poSuppliers`. This list feeds the supplier autocomplete in the PO Supplier Breakdown add/edit modal, and off-list supplier values are rejected on save.

**PO Suppliers template and import:** the PO Suppliers tab provides:
- **Template download:** `PO_Suppliers_Template.xlsx` with one required column: `Supplier Name`.
- **Import:** accepts `.xlsx/.xls/.csv` (first sheet), reads supplier names from `Supplier Name` (fallback headers: `supplierName`, `Supplier`, `supplier`), and appends valid suppliers.
- **Duplicate guard:** case-insensitive, trimmed comparison; skips names already in Admin and duplicate names repeated within the same import file.
- **Result panel:** shows `added` count and per-row skip reasons.
...
- **Conflicts:** matched by `jobNumber` (if both have one) or `siteId + siteName`. One modal decision applies to all conflicts at once:
  ...
  - **Add New + Override All:** adds new sites and replaces all conflicting entries with VO data (existing site comment is preserved).
- Deduplication: same job number or same siteId+siteName blocks duplicate syncing.

**Settings > Routine JSON Backup:** settings include `routineBackupEnabled` and `routineBackupTime` under `globalData.settings`. `AdminView.vue` renders an enable switch plus `<input type="time">`, a live "Next export in ..." countdown, and a reset modal when changing time after today's run/cancel. The modal offers **Reset Today** (clears `routineBackupLastRunDate`) or **Wait Until Next Day**. `App.vue` owns the runtime scheduler and, while the SPA is open, shows a 15-second warning then calls `downloadFullBackup(store.vos.value, store.invoicePrepIds.value, filename)` once per local calendar day. Output filename: `VariationTracker_Backup_YYYY-MM-DD_HH-MM.json`. The backup uses the same full restore-ready payload as Import/Export Backup & Restore. Last run/cancel date persists in localStorage as `routineBackupLastRunDate`.
