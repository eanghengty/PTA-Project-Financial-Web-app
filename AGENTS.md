...
**KPI cards (6):** Total/Visible Sites · Started (count + cost to complete) · Not Started (count + cost to complete) · Cancel (count + cost to complete) · Other (count + cost to complete) · Total/Visible Cost to Complete. All KPI values respect the active month filter, and they also update from the active search/status/scope filter set. When any filter is active, the first and last cards switch to the visible subset.

**Filter bar:** Text search (site ID / name / job) + All / Started / Not Started / Cancel / Other status button group + **Scope dropdown** (emerald when active) + **Month dropdown** (violet, calendar icon — shows only months that have cost entries with dates; turns violet when active).
...
- **Template** — downloads `Site_Status_Import_Template.xlsx` pre-filled with all current site IDs and current status text. Includes blank-state example rows covering `Started`, `Not Started`, `Cancel`, and `Other` when no sites exist.
- **Import Status** — file picker (xlsx/xls/csv). Columns: `Site ID` (required), `Status` (required). Accepted status values: `Started / Yes / 1 / True / start` → `started`; `Not Started / Not-Started / Not Start / No / 0 / False` → `not-started`; `Cancel / Cancelled / Canceled` → `cancel`; `Other` → `other`. Blank rows silently skipped. After import, a dismissible **result log panel** appears above the KPI cards:
