      ...
      <div class="w-full px-4 lg:px-6 h-14 flex items-center gap-4">
        ...
        <nav class="flex items-center gap-0.5 flex-1 min-w-0">
          ...
          >
            ...
            <span
              v-if="item.key === 'invoice-list' && invoicePrepCount > 0"
              class="px-1.5 py-0.5 rounded-full text-[10px] font-bold leading-none"
            ...
            <span
              v-if="item.key === 'reminders' && reminderCount > 0"
              class="px-1.5 py-0.5 rounded-full text-[10px] font-bold leading-none"
      ...
      <transition name="routine-toast">
        ...
        >
          ...
          <div class="h-1 bg-gray-100">
            <div class="h-full" :class="routineBackupToast.type === 'success' ? 'bg-emerald-500' : 'bg-slate-400'"></div>
          </div>
        </div>
      </transition>
    </Teleport>
...
const currentView = computed(() => route.meta?.viewKey || 'dashboard')
...
function goToView(viewKey) {
  const path = VIEW_PATHS[viewKey] || VIEW_PATHS.dashboard
